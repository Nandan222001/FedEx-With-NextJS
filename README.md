...
---
# 📦 FedEx Shipping Rate Integration with Next.js (App Router)

A modern shipping rate calculator built using **Next.js (App Router)** and integrated with **FedEx Rate API (OAuth2)**. Easily test FedEx shipping rates from the US using your sandbox credentials.

---

## 🚀 Features

- ✅ OAuth2 integration with FedEx Developer API
- 📦 Shipping rate quotes via `/rate/v1/rates/quotes`
- 🧪 Postman-compatible API testing
- 💡 Error handling & JSON validation
- 🔒 Secure token flow using `.env.local`
...
├── app/
│ └── api/
│ └── fedex-rate/ # API route: POST /api/fedex-rate
│ └── route.js
├── lib/
│ └── fedex.js # Handles OAuth token fetch
├── components/
│ └── RateForm.jsx # (Optional UI component for shipping form)
...
- Or a dark-mode friendly version of this doc

I can generate and auto-publish those too.